...
namespace IAE.Microservice.Api.Infrastructure
{
    public class JsonWithFilesModelBinder : IModelBinder
    {
        ...
        public JsonWithFilesModelBinder(
            IOptions<MvcNewtonsoftJsonOptions> jsonOptions, ILoggerFactory loggerFactory)
        {
        ...
        public async Task BindModelAsync(ModelBindingContext bindingContext)
        {
            ...
            var model = JsonConvert.DeserializeObject(
                rawValue, bindingContext.ModelType, _jsonOptions.Value.SerializerSettings);

            // Now, bind each of the IFormFile properties from the other form parts
            foreach (var property in bindingContext.ModelMetadata.Properties)
            {
                if (property.ModelType != typeof(IFormFile) && property.ModelType != typeof(List<IFormFile>))
                    continue;

                var fieldName = property.BinderModelName ?? property.PropertyName;
                var propertyModel = property.PropertyGetter(bindingContext.Model);
                ModelBindingResult propertyResult;
                using (bindingContext.EnterNestedScope(property, fieldName, fieldName, propertyModel))
                {
                ...
                else if (property.IsBindingRequired)
                {
                    ...
                    bindingContext.ModelState.TryAddModelError(fieldName, message);
                }
            }
